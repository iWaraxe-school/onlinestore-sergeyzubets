...
public class PresentProductsAsTable {
    ...
    public String getProductsAsTableAsPerLocale(List<Product> listOfProducts, Locale locale) {
        List<List<String>> rows = new ArrayList<>();
        rows.add(Arrays.asList("Product Name", "Rate", "Price, " + Currency.getInstance(locale)));
        formulateList(rows, listOfProducts);
